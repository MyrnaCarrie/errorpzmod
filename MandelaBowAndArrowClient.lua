...
Client.simpleFeatherFletchings = true;
Client.sayOof = true;
...
function Client.addArcheryXP(character, XP)
	if Shared.versionNumber ~= nil and Shared.versionNumber < 41.51 and Shared.versionNumber > 40  then
		...
		--if isClient() then
		--	sendClientCommand("MandelaBowAndArrow", "AddArcheryXP", {XP});
		--else
...
function Client.getMovingTargetAim(player, target, weaponOffset, missileSpeed)
	...
	if distance > 1 then
		local discriminant = (gravity*((gravity*(distance^2))+(2*zDifference*(missileSpeed^2))));
		if discriminant < 0 then
			if Client.sayOof then player.Say("Oof") end
			return pitch, yaw;
		else
			pitch = (math.atan(((missileSpeed^2)-math.sqrt((missileSpeed^4)-discriminant))/(gravity*distance)));
		end
	end
	...
	for i=1, 2, 1 do
		...
		if distance > 1 then
			--pitch = (math.atan(((missileSpeed^2)-math.sqrt((missileSpeed^4)-(gravity*((gravity*(distance^2))+(2*zDifference*(missileSpeed^2))))))/(gravity*distance)));
			local discriminant = (gravity*((gravity*(distance^2))+(2*zDifference*(missileSpeed^2))));
			if discriminant < 0 then
				if Client.sayOof then player.Say("Oof") end
				return pitch, yaw;
...
function Client.onPlayerUpdate(player)
	...
	local weapon = player:getPrimaryHandItem();
	if weapon ~= nil then
		if Shared.Bows[weapon:getFullType()] ~= nil then
	...
	if player:isLocalPlayer() then
		...
		if modData.MandelaBowAndArrowArrowNumber > 0 then
			local ArrowNumber = modData.MandelaBowAndArrowArrowNumber;
			--local timeMultiplier = getGameTime():getMultiplier() * getGameTime():getTimeDelta(); --Oof this would need some slight recoding
				...
				if ArrowItem == nil then
					...
					for i2=1,divisions do
						if Collision == false then
							...
							if ArrowItem:getRecoilpad() and Shared.ArrowPartData[ArrowItem:getRecoilpad():getFullType()] ~= nil and Shared.ArrowPartData[ArrowItem:getRecoilpad():getFullType()].fluflu == true then
							...
							if Square ~= nil then
								...
								--[[
									...
									local movingObjects = Square:getObjects();
									for ii=0, movingObjects:size()-1 do
										--if(player:getZ() < 3) then u wot, m8?
											local collision = false;
											local wall = false;
...
do
	local Nihilist_s_Silencers_patched = false
	local function NihilistsSilencersPatch()
		print("OnCreateSurvivor: Bow and Arrow mod patching ISReloadWeaponAction.attackHook of NihilistsSilencers");
		Hook.Attack.Remove(ISReloadWeaponAction.attackHook);
		local nihilist_attackHook = ISReloadWeaponAction.attackHook;
		ISReloadWeaponAction.attackHook = function(character, chargeDelta, weapon)
			if Client.isMandelaBow(weapon:getFullType()) then
				if not Client.attackHook(character, chargeDelta, weapon) then
					nihilist_attackHook(character, chargeDelta, weapon);
				end
			else
				nihilist_attackHook(character, chargeDelta, weapon);
			end
		...
		Nihilist_s_Silencers_patched = true
	end
	...
	function Client.OnCreateSurvivor(Survivor) --fails
		...
		if Client.Nihilist_s_Silencers_Active and not Nihilist_s_Silencers_patched then --Nihilist's Silencers Patch. Nihilist's Silencers is designed to override attackHook mod code. Perhaps this could go in Client.OnLoad.
			NihilistsSilencersPatch();
		end
	end
end
...
do
	...
	Client.attackHook = function(character, chargeDelta, weapon)
		if (Client.isMandelaBow(weapon:getFullType()) and (((Shared.versionNumber == nil or Shared.versionNumber >= 41.60 or Shared.versionNumber <= 40) and (not character:isDoShove())) or ((Shared.versionNumber ~= nil and Shared.versionNumber < 41.60) and (not character:isShoving())))) then
			if Client.alternativeDrawing then
				Shared.getModData(character).isDrawing = true
				Shared.getModData(weapon).drawnAmount = 0
				--Suddenly ceasing working on this but it's okay because that boolean above is set to false
				return true
			else
				return Client.tryShoot(character, weapon)
			end
		else
			return false
		end
	...
	function Client.tryShoot(character, weapon)
		if Shared.getModData(weapon).isDrawn then
			...
			if ISMBAReloadBowAction.canShoot(weapon) then
				--local missileSpeed = Shared.Bows[weapon:getFullType()].speed;
				...
				local soundVolume = Shared.Bows[weapon:getFullType()].soundVolume or 1;
				local soundRadius = Shared.Bows[weapon:getFullType()].soundRadius or 4;
	...
	do
		...
		local original_attackHook = ISReloadWeaponAction.attackHook; --Also stores a reference to it in a local variable
...
function Client.getNecessaryDrawnTime(playerObj, bow)
	return 0.6
end
...
function Client.setUpBow(player, bow)
	...
	if player:isAiming() then
		...
		if not weaponModData.isDrawn then
			drawnAmount = weaponModData.drawnAmount;
			if drawnAmount == nil then drawnAmount = 0; end
			if Client.alternativeDrawing then
				if playerModData.isDrawing then
					drawnAmount = drawnAmount + getGameTime():getTimeDelta();
				end
			else
				drawnAmount = drawnAmount + getGameTime():getTimeDelta();
			end
			if drawnAmount < Client.getNecessaryDrawnTime(player, bow) then
				weaponModData.drawnAmount = drawnAmount;
			else
				weaponModData.drawnAmount = 0;
...
function Client.OnEquipPrimary(player, equipedItem)
	...
	local bows = {};
	for bowType,v in pairs(Shared.Bows) do
...
--FirearmsB41 Patch. FirearmsB41 uses ISReloadWeaponAction.attackHookFirearms instead of ISReloadWeaponAction.attackHook, so it needs a different compatibility function for it. Also, the function runs in OnGameBoot, which should be after all the lua has loaded.
...
Client.Nihilist_s_Silencers_Active = false;
function Client.OnGameBoot()
	...
	Client.SetAttachmentModels()
	if getActivatedMods():contains("NihilistsSilencers") then
		Client.Nihilist_s_Silencers_Active = true;
	end
end

function Client.SetAttachmentModels()
	for i = 1,#Shared.WeaponWithAttachmentList, 1 do --For each weapon in this list
		local weaponScriptItem = ScriptManager.instance:getItem(Shared.WeaponWithAttachmentList[i]); --Gets the script item that the item instances are based on
		for weaponPart, model in pairs (Shared.AttachmentModels) do --Goes through the pairs of item and model name
			if weaponpart ~= "MandelaBowAndArrow.MandelaAttachmentFletchingsDucttapeFluFlu" and weaponpart ~= "MandelaBowAndArrow.MandelaAttachmentFletchingsDucttape" then
				weaponScriptItem:DoParam("ModelWeaponPart = " .. weaponPart .. " " .. model .. " arrow arrow");
			end
		end
	end
end
...
function Client.OnLoad()
	if getActivatedMods():contains("BrutalHandwork") then
		print("OnGameBoot: Bow and Arrow mod patching attack hook of Brutal Handwork");
	end
end
...
function Client.OnFillMenu(player, context, items)
	...
	for i=#context.options, 1, -1 do
		...
		if option.name and option.target and option.name == removeUpgradeName then --(testItem:getFullType() == "MandelaBowAndArrow.MandelaArrowBlank" or Client.isMandelaBow(item:getFullType(), 1)) and 
				...
				if type(target) == "table" then
					for i3,item in ipairs(target.items) do
						if Client.isMandelaBow(item:getFullType(), 1)
						 or item:getFullType() == "MandelaBowAndArrow.MandelaArrowBlank" then
...
function Client.SetBowAndArrowModel(player, item, number, isServerCommand)
	if Shared.getModData(item).modelVariantNumber ~= number then
		...
		local modelName = Shared.Bows[itemFullType].model;
		if number == 0 then
...
function Client.CustomRecipesMenu(player, context, items)
	...
	if testItemFullType == "MandelaBowAndArrow.MandelaArrowBlank" or Shared.ArrowPartData[testItemFullType] ~= nil then
		local arrowShafts = {};
		local arrowsNoHead = {};
		...
		for i=1,containers:size() do
			...
			if not alreadySearched then
				...
				for j=1,container:getItems():size() do
					...
					if Shared.ArrowPartData[itemFullType] then
						if Shared.ArrowPartData[itemFullType].sort == "Head" then
							AddToItemTypeList(arrowHeads, item, itemFullType)
						elseif Shared.ArrowPartData[itemFullType].sort == "Fletchings" then
...
function ISCraftAction:perform()
	...
	if Client.simpleFeatherFletchings then
		if self.recipe == ScriptManager.instance:getRecipe("Butcher Bird") then
			local inv = self.character:getInventory()
			for i=1, 4 do
				inv:AddItem("MandelaBowAndArrow.MandelaFletchingsFeathers");
			end
		end
	end
end
