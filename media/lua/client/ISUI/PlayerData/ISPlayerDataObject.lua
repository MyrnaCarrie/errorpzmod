...
--    print("createInventoryInterface player="..self.id);
    panel2.player = self.id;
--		changed to not make visible tranferall & loot all buttons
    panel2.transferAll:setVisible(false);
...
--		changed to not make visible tranferall & loot all buttons
    panel3.lootAll:setVisible(false);
    ...
    if isMouse then
		...
		panel2.pin = true;
        panel2.collapseButton:setVisible(true);
        panel2.pinButton:setVisible(false);
        panel2.pinButton:bringToTop();
        panel2.isCollapsed = false;
        --panel2:setMaxDrawHeight(300);
        
        panel3.pin = true;
        panel3.collapseButton:setVisible(true);
        panel3.pinButton:setVisible(false);
        panel3.pinButton:bringToTop();
        panel3.isCollapsed = false;
        --panel3:setMaxDrawHeight(300);
    else
        panel2:setPinned()
        panel3:setPinned()
    end
...
-- changed inventory x y to be closer to the player
	local newWidth = 360
	local off = round((w - newWidth*2) / 3)

    if mouse then
--
		local off = round((w - 720) / 3)
        local divhei = h / 3;
        ...
        self.x1left = x;
        self.y1top = y;
			self.y1 = self.y1top + (h/3);
			self.w1 = newWidth;
			self.h1 = divhei;
		self.x2 = self.x1 + newWidth + off;
		self.y2 = self.y1;
		self.w2 = newWidth;
		self.h2 = divhei;
	...
	if playerID > 0 then
		...
		getSpecificPlayer(playerID):setOffSetXUI(self.x2);
