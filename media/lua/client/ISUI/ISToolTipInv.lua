...
--	changed for tooltip to follow inventory position
	if not self.followMouse then
		self.tooltip:setX(mx)
		self.tooltip:setY(my - self:getHeight())
		self:setX(mx)
		self:setY(my - self:getHeight())
	end
	...
	self:drawRect(0, 0, self.width, self.height, 0.2, 0.08, 0.05, 0.03);
...
function ISToolTipInv:new(item)
   ...
   o.backgroundColor = {r=0.08, g=0.05, b=0.03, a=0.2};
