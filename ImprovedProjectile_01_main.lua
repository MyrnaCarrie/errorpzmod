...
local function createInfoTableServer(infoTable)
	local infoTableNew = {
		...
		infoTable[16],						-- [8]
		infoTable[5][1],					-- [9]
		ImprovedProjectile.gameTimeMult		-- [10]
	}
	return infoTableNew
end
...
--*************************************************************************************--
function IPPJcheckTarget(player, coord, cosT, sinT, speed, projId, hitbox, pvp, originZ)
	local zombieTable	= {}
	local playerTable	= {}
	local hitBoxX		= speed
	...
	local z = originZ -- math.floor(coord[3])

	for ix, vx in pairs(x) do
		for jy, vy in pairs(y) do
			--local square = getCell():getOrCreateGridSquare(vx, vy, z)
			local zDiff = 0
			for iz = math.floor(z), 0, -1 do
				zDiff = z - iz
				square = getCell():getOrCreateGridSquare(vx - (3 * zDiff), vy - (3 * zDiff), iz)
				if square:TreatAsSolidFloor() then
					if iz < z then
						local noRoof = true
						if not square:isCouldSee(0) then
							local vx_u = square:getX()
							local vy_u = square:getY()
							local vz_u = square:getZ() + 1
							for j = vz_u, z do
								local tempSquare = getCell():getOrCreateGridSquare(vx_u, vy_u, j)
								if tempSquare:TreatAsSolidFloor() then
									noRoof = false
									break
								end
							end
						...
						if noRoof then
							...
							for i = 1, objects:size() do
								...
								if instanceof(movingObject, "IsoZombie") and movingObject:isAlive() and (not movingObject:getModData().IPPJHitBy or movingObject:getModData().IPPJHitBy ~= projId) then
									table.insert(zombieTable, {movingObject, zDiff})
											...
											if not (SandboxVars.ImprovedProjectile.IPPJEnableNonPVPZone and NonPvpZone.getNonPvpZone(vx, vy)) then
												...
												if targetFaction and playerFaction and playerFaction == targetFaction then
													if SandboxVars.ImprovedProjectile.IPPJIgnoreFactionPVP or (pvp[3] and movingObject:isFactionPvp()) then
														table.insert(playerTable, {movingObject, zDiff})
													end
												else
													table.insert(playerTable, {movingObject, zDiff})
												end
						...
						for i = 1, objects:size() do
							local movingObject = objects:get(i - 1)
							if instanceof(movingObject, "IsoZombie") and movingObject:isAlive() and (not movingObject:getModData().IPPJHitBy or movingObject:getModData().IPPJHitBy ~= projId) then
								table.insert(zombieTable, {movingObject, 0})
							elseif instanceof(movingObject,"IsoPlayer") and movingObject ~= player then
								-- Multiplayer PVP
								if isClient() then
									if pvp[4] and (pvp[5] or (pvp[2] and not movingObject:getSafety():isEnabled()) or (getServerOptions():getBoolean("PVP") and not getServerOptions():getBoolean("SafetySystem"))) then
										if not (SandboxVars.ImprovedProjectile.IPPJEnableNonPVPZone and NonPvpZone.getNonPvpZone(vx, vy)) then
											...
											if targetFaction and playerFaction and playerFaction == targetFaction then
												if SandboxVars.ImprovedProjectile.IPPJIgnoreFactionPVP or (pvp[3] and movingObject:isFactionPvp()) then
													table.insert(playerTable, {movingObject, 0})
												end
											else
												table.insert(playerTable, {movingObject, 0})
											end
	...
	for i, v in pairs(zombieTable) do
		local movX	= v[1]:getX() - (coord[1] - (3 * v[2]))
		local movY	= v[1]:getY() - (coord[2] - (3 * v[2]))
		...
		if rotX >= -0.1 and rotX <= hitBoxX and rotY <= hitbox[2] then
			...
			if rotY < hitbox[5] then
				if distance < playerCheck[2] then
					playerCheck = {v[1], distance, 1}
				end
			elseif rotY < hitbox[6] and playerCheck[3] < 0.8 then
				if distance < playerCheck[2] then
					playerCheck = {v[1], distance, 0.6}
				end
			elseif playerCheck[3] < 0.4 then
				if distance < playerCheck[2] then
					playerCheck = {v[1], distance, 0.2}
				end
			end
		end
	end
	...
	if player:isAiming() and self.isValid and self.currInfo["weaponName"] == weapon:getFullType() then
		...
		if pUpdateTick < 1 then
			...
			if isKeyDown(getCore():getKey("ManualFloorAtk")) then
				self.aimHeightOff = heightOff
			elseif IPPJSettings.AimAssistLZombie then
				local tmpSquare = getCell():getOrCreateGridSquare(aimX, aimY, playerZ)
				if not tmpSquare:TreatAsSolidFloor() then
					while playerZ > 0 do
						playerZ = playerZ - 1
						aimX = aimX - 3
						aimY = aimY - 3
						local tmpSquare2 = getCell():getOrCreateGridSquare(aimX, aimY, playerZ)
						if tmpSquare2:TreatAsSolidFloor() then break end
					end
				end
				...
				for ix, vx in pairs(x) do
					for jy, vy in pairs(y) do
						local square = getCell():getOrCreateGridSquare(vx, vy, playerZ)
						if square:TreatAsSolidFloor() then
...
local function drawText()
	...
	if ImprovedProjectile.crossHair.instance then
		...
		local textYOff = getMouseY() + ImprovedProjectile.aimHeightOff - IPPJSettings.CrosshairHeight / ZOOM + 35
		...
		if v[4] then
			...
			if checkRemoveProjectile ~= 3 and v[4]:IsOnScreen() then
				if doSimplify == false then
					v[1] = ImprovedProjectile.createProjectile(v[4], v[2], v[5][1], v[7], v[1])
				else
			...
			-----------------------------------------------------------------------------
			local zombieTable, hitPlayer, zombieNum = IPPJcheckTarget(v[15], v[7], v[5][2], v[5][3], v[10][1], v[16], v[20], v[21], v[6][3])
			...
			if v[11] <= 0 then
				...
				-- Attach bow or slingshot projectile to zombie
				if v[3] == "XBow" then
					local proj			= InventoryItemFactory.CreateItem(v[14])
					local bust			= GUNFIGHTER.OPTIONS.options.dropdown135 * 2
					local broke			= false
					-- Projectile broken
					if ZombRand(bust) == 2 then
						if proj:getModData().Break ~= nil then
							proj	= InventoryItemFactory.CreateItem(proj:getModData().Break)
							broke	= true
						end
					end
					if isClient() then
						sendClientCommand("IPPJ", "attachProjZombie", {v[15]:getOnlineID(), lastZombie:getOnlineID(), {lastZombie:getX(), lastZombie:getY(), lastZombie:getZ()}, v[14], broke})
					end
					if lastZombie and lastZombie:isAlive() then
						if lastZombie:getModData().stuck_Body01 == nil then
							lastZombie:setAttachedItem("Stuck Body01", proj)
							lastZombie:getModData().stuck_Body01 = 1
						elseif	lastZombie:getModData().stuck_Body02 == nil then
							lastZombie:setAttachedItem("Stuck Body02", proj)
							lastZombie:getModData().stuck_Body02 = 1
						elseif	lastZombie:getModData().stuck_Body03 == nil then
							lastZombie:setAttachedItem("Stuck Body03", proj)
							lastZombie:getModData().stuck_Body03 = 1
						elseif	lastZombie:getModData().stuck_Body04 == nil then
							lastZombie:setAttachedItem("Stuck Body04", proj)
							lastZombie:getModData().stuck_Body04 = 1
						elseif	lastZombie:getModData().stuck_Body05 == nil then
							lastZombie:setAttachedItem("Stuck Body05", proj)
							lastZombie:getModData().stuck_Body05 = 1
						elseif	lastZombie:getModData().stuck_Body06 == nil then
							lastZombie:setAttachedItem("Stuck Body06", proj)
							lastZombie:getModData().stuck_Body06 = 1
						else
							lastZombie:getCurrentSquare():AddWorldInventoryItem(proj, 0.0, 0.0, 0.0)
						end
					else
						--print("[IPPJ] Projectile to Zombie Inv")
						lastZombie:getInventory():AddItem(proj)
					end
				end
				ImprovedProjectile.removeProjectile(v[1])
				projTable[iv] = nil
			...
			-----------------------------------------------------------------------------
			if checkPorj and ImprovedProjectile.checkBlocked(v[4], v[5][1], v[7], {v[10][2], v[10][3], v[10][1]}, v[19], v[15], v[18], v[17], fgRoll) then
				if isClient() and checkRemoveProjectile == 1 then
					sendClientCommand("IPPJ", "removeProjectile", {v[15]:getOnlineID(), v[16]})
				end
				-- Bow or Slingshot drops projectile
				if v[3] == "XBow" then
					local proj			= InventoryItemFactory.CreateItem(v[14])
					local bust			= GUNFIGHTER.OPTIONS.options.dropdown135 * 2
					...
					-- Projectile must dropped on Solid Floor
					for i = v[7][3], 0, -1 do
						projSquare = getCell():getOrCreateGridSquare(v[7][1], v[7][2], i)
						if projSquare:TreatAsSolidFloor() and not projSquare:getProperties():Is(IsoFlagType.water) then
							local objs = projSquare:getObjects()
							local surface = 0
							...
							projSquare:AddWorldInventoryItem(proj, 0.0, 0.0, surface * 0.01)
							break
						end
					end
				end
...
function ImprovedProjectile.calcWeaponRecoil(player, weapon)	-- calcRecoilDelay from Gunfighter @Aresnal[26]
	...
	elseif	string.find(weapon:getType(), "Fold")	then	stock	= 0
	elseif	string.find(weapon:getType(), "Pistol") then	stock	= 0
	...
	if weapon:isRanged() then
		...
		if		ammo	== "Base.BB177"				then	recoil	= -10
		elseif	ammo	== "Base.PB68"				then	recoil	= -10
		elseif	ammo	== "Base.Bullets22"			then	recoil	= -4
		elseif	ammo	== "Base.Bullets57"			then	recoil	= -2
		elseif	ammo	== "Base.Bullets380"		then	recoil	= 1
		elseif	ammo	== "Base.Bullets38"			then	recoil	= 3
		elseif	ammo	== "Base.Bullets9mm"		then	recoil	= 4
		elseif	ammo	== "Base.Bullets45"			then	recoil	= 6
		elseif	ammo	== "Base.Bullets357"		then	recoil	= 8
		elseif	ammo 	== "Base.Bullets45LC"		then	recoil	= 10
		elseif	ammo	== "Base.Bullets44"			then	recoil	= 12
		elseif	ammo	== "Base.Bullets50MAG"		then	recoil	= 16
		elseif	ammo	== "Base.Bullets4570"		then	recoil	= 18
		elseif	ammo	== "Base.223Bullets"		then	recoil	= 13
		elseif	ammo	== "Base.556Bullets"		then	recoil	= 14
		elseif	ammo	== "Base.545x39Bullets"		then	recoil	= 15
		elseif	ammo	== "Base.762x39Bullets"		then	recoil	= 16
		elseif	ammo	== "Base.308Bullets"		then	recoil	= 20
		elseif	ammo 	== "Base.762x51Bullets"		then	recoil	= 20
		elseif	ammo	== "Base.762x54rBullets"	then	recoil	= 21
		elseif	ammo	== "Base.3006Bullets"		then	recoil	= 22
		elseif	ammo	== "Base.410gShotgunShells"	then	recoil	= 18
		elseif	ammo	== "Base.20gShotgunShells"	then	recoil	= 24
		elseif	ammo	== "Base.ShotgunShells"		then	recoil	= 26
		elseif	ammo	== "Base.10gShotgunShells"	then	recoil	= 30
		elseif	ammo	== "Base.4gShotgunShells"	then	recoil	= 40
		elseif	ammo	== "Base.50BMGBullets"		then	recoil	= 56
		-- Above : Gunfighter's / Below : Others
		elseif	ammo	== "Base.Nails"				then	recoil	= -2
		else -- Recoil based on damage
		...
		if		weapon:getFireMode() == "[6]Rotary" then
				recoil	= (recoil + 40)  / 4
				weight	= (weight + 200) / 20
				base	= 22
		elseif	weapon:hasTag("Fixed") then
				recoil	= (recoil + 50)  / 5
				weight	= (weight + 300) / 20
				base	= 20
		end

		if	weapon:hasTag("Grip") then	recoil = recoil - 2 end
		if	weapon:hasTag("Comp") then	recoil = recoil - 2 end
...
function ImprovedProjectile:calcRecoil(weapon)
	...
	if not self.currInfo["fireMode"] or self.currInfo["fireMode"] ~= fireMode then
		if not fireMode or string.find(fireMode, "Single") then
			if		fireMode and string.find(fireMode, "[H]") then	self.currInfo["fmRecoil"] = 2.0 + (10 - self.Aiming) * 0.05
			else													self.currInfo["fmRecoil"] = 1.5 + (10 - self.Aiming) * 0.05
			end

		elseif string.find(fireMode, "Burst") then
			if		string.find(fireMode, "2]") then	self.currInfo["fmRecoil"] = 0.75
			elseif	string.find(fireMode, "3]") then	self.currInfo["fmRecoil"] = 0.65
			else										self.currInfo["fmRecoil"] = 0.75
			end

		elseif string.find(fireMode, "Auto") then
			self.currInfo["fmRecoil"] = 1.15

		elseif fireMode == "[6]Rotary" then
			if weapon:getAmmoType() == "Base.50BMGBullets" then
				self.currInfo["fmRecoil"] = 0.6
			else
				self.currInfo["fmRecoil"] = 0.8
			end
		else
			self.currInfo["fmRecoil"] = 1.1
		end
	end

	self.currInfo["recoil"] = self.currInfo["fmRecoil"] * ((weapon:getModData().IPPJRecoilDelay + (weapon:getModData().IPPJRecoilDelayParts * 1.5)) / self.currInfo["recoilDenom"])
...
function ImprovedProjectile:calcAiming(player, weapon)
	...
	local maxRange = (SandboxVars.ImprovedProjectile.IPPJSniperScope == 1) and IPPJSettings.SniperScopeRange or SandboxVars.ImprovedProjectile.IPPJSniperScopeRange
...
function ImprovedProjectile:onShootWeapon(player, weapon)
	...
	if weapon:getModData().IPPJHitChanceParts then
		aimRate = math.max(aimRate - (weapon:getModData().IPPJHitChanceParts * 0.5), 0)
	end
	...
	local projCritChance	= weapon:getCriticalChance() + self.currInfo["critChance"]
	if self.currInfo["weaponType"] == "XBow" then
		projCritChance = projCritChance + self.Aiming * 4
	else
		projCritChance = projCritChance + self.Aiming * 1.5
	end
	...
	local projInfo = {
		...
		weaponType,						-- [3]  Weapon type : Rifle or Shotgun or XBow
	...
	-- Buffs bow and slingshot damage
	if projInfo[3] == "XBow" then
		projInfo[8] = projInfo[8] + SandboxVars.ImprovedProjectile.IPPJBuffBowSlingshot
	end
	...
	if player:getModData().TempThrown and player:getModData().TempThrown == weapon:getAmmoType() then
